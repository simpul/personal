window.onload = function () {
    layui.use(['element', 'form', 'jquery', 'laypage'], async function () {
        ...
        await $.ajax({
            type: "GET",
            url: "http://localhost:3000/photo-query",
            success: function (data) {
                $("#pages").data("max", data.maxNum);
                // data.maxNum data.artList
                data.artList.forEach(function(v, i, arr){
                    document.getElementById("photo-content").innerHTML += `<li class="pr">
        ...
        laypage.render({
            ...
            curr: location.pathname.replace("/photo/", ""),
            jump(obj, f) {
                $("#pages a").each((i, v) => {
                    let pageValue = `/photo/${$(v).data("page")}`;
                    v.href = pageValue;
                });
            }
        });
